...
Standalone single-instance **Redis 8** (`redis:8-alpine`) for local development, deployed into the `furseal` namespace. It serves as a backing store for queued jobs and is configured with password authentication plus a non-evicting eviction policy so queue data does not disappear under load.
...
```bash
kubectl apply -f k8s/redis-secret.yaml      # namespace furseal, type furseal
...
PASS=$(kubectl get secret redis-secret -n "$NAMESPACE" -o jsonpath='{.data.REDIS_PASS}' | base64 -d) && \
kubectl exec "pod/$POD" -n "$NAMESPACE" -- redis-cli -a "$PASS" ping      # expect: PONG

# Inspect memory and config (same $PASS pattern applies):
kubectl exec "$POD" -n "$NAMESPACE" -- redis-server --version             # Redis 8.x
kubectl exec "$POD" -n "$NAMESPACE" -- redis-cli -a "$PASS" INFO MEMORY
kubectl exec "$POD" -n "$NAMESPACE" -- redis-cli -a "$PASS" CONFIG GET maxmemory-policy    # noeviction

# Local data directory is gitignored — to reset persistence:
kubectl delete pod "$POD" -n "$NAMESPACE"             # Pod will recreate with emptyDir hostPath
